...
static const char * const adf4377_muxout_modes[] = {
	...
	[ADF4377_MUXOUT_LKDET] = "muxout_lock_detect",
...
static int adf4377_soft_reset(struct adf4377_state *st)
{
	...
	unsigned int read_val, timeout;
	int ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x0),
	...
	timeout = 100;
	do {
		ret = regmap_read(st->regmap, ADF4377_REG(0x0), &read_val);
		if (ret)
			return ret;

		if (!(read_val & (ADF4377_SOFT_RESET_R_MSK | ADF4377_SOFT_RESET_R_MSK)))
			return 0;

		/* The ADF4377 requires typically 200us to power up and settle */
	} while (--timeout);
...
static int adf4377_set_default(struct adf4377_state *st)
{
	...
	ret = regmap_write(st->regmap, ADF4377_REG(0x0f), FIELD_PREP(ADF4377_R00F_RSV1_MSK, 0x14));
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x1c),
				 ADF4377_R01C_RSV1_MSK,
				 FIELD_PREP(ADF4377_R01C_RSV1_MSK, 0x1));
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x1f),
				 ADF4377_R01F_RSV1_MSK,
				 FIELD_PREP(ADF4377_R01F_RSV1_MSK, 0x7));
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x20),
				 ADF4377_R020_RSV1_MSK,
				 FIELD_PREP(ADF4377_R020_RSV1_MSK, 0x1));
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x21), ADF4377_R021_RSV1);
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x22), ADF4377_R022_RSV1);
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x23), ADF4377_R023_RSV1);
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x25),
				 ADF4377_R025_RSV1_MSK,
				 FIELD_PREP(ADF4377_R025_RSV1_MSK, 0x16));
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x2C), ADF4377_R02C_RSV1);
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x31), ADF4377_R031_RSV1);
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x32),
				 ADF4377_R032_RSV1_MSK,
				 FIELD_PREP(ADF4377_R032_RSV1_MSK, 0x9));
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x33), ADF4377_R033_RSV1);
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x34), ADF4377_R034_RSV1);
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x3A), ADF4377_R03A_RSV1);
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x3B), ADF4377_R03B_RSV1);
	if (ret)
		return ret;

	return regmap_write(st->regmap, ADF4377_REG(0x42), ADF4377_R042_RSV1);
}

int adf4377_set_freq(struct adf4377_state *st)
{
	unsigned int timeout, read_val;
	int ret;
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x11),
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x2E),
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x2F),
				 ADF4377_DCLK_DIV1_MSK,
				 FIELD_PREP(ADF4377_DCLK_DIV1_MSK, st->dclk_div1));
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x24), ADF4377_DCLK_MODE_MSK,
				 FIELD_PREP(ADF4377_DCLK_MODE_MSK, st->dclk_mode));
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x27),
			   FIELD_PREP(ADF4377_SYNTH_LOCK_TO_LSB_MSK, st->synth_lock_timeout));
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x28),
	...
	if (st->f_out > ADF4377_MAX_CLKPN_FREQ || st->f_out < ADF4377_MIN_CLKPN_FREQ)
		return -EINVAL;

	st->f_vco = st->f_out;

	while (st->f_vco < ADF4377_MIN_VCO_FREQ) {
		st->f_vco <<= 1;
		st->clkout_div_sel++;
	}

	st->n_int = div_u64(st->f_out, st->f_pfd);
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x12),
	...
	ret = regmap_write(st->regmap, 0x10,  FIELD_PREP(ADF4377_N_INT_LSB_MSK, st->n_int));
	if (ret)
		return ret;

	timeout = 100;
	do {
		ret = regmap_read(st->regmap, ADF4377_REG(0x49), &read_val);
		if (ret)
			return ret;

		if (!(read_val & (ADF4377_FSM_BUSY_MSK)))
			break;
	...
	return regmap_update_bits(st->regmap, ADF4377_REG(0x19),
...
static int adf4377_init(struct adf4377_state *st)
{
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x15), ADF4377_CP_I_MSK,
				 FIELD_PREP(ADF4377_CP_I_MSK, ADF4377_CP_10MA1));
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x00),
	...
	/* Power Up */
	ret = regmap_write(st->regmap, ADF4377_REG(0x1a),
...
static ssize_t adf4377_read(struct iio_dev *indio_dev,
			    uintptr_t private,
...
static ssize_t adf4377_write(struct iio_dev *indio_dev,
			     uintptr_t private,
			     const struct iio_chan_spec *chan,
				   ...
				   unsigned int mode)
{
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x1D),
				 ADF4377_MUXOUT_MSK,
				 FIELD_PREP(ADF4377_MUXOUT_MSK, mode));
	if (ret)
		return ret;

	st->muxout_select = mode;
